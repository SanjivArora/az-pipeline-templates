...
#		if(CheckComputerState $server){
            ...
            #Apply latest SCOM Agent Patch UR5
            LogMessage "Applying latest SCOM Agent Patch"
            $latestfolder = gci "\\vmmh1mgt001.healthcare.huarahi.health.govt.nz\packages$\Software Distribution\SCOM\Software\Agent-2019\Patch" | sort -Property LastWriteTime -Descending | select -First 1
            ...
            #Wait for SCOM agent patch to complete before updating VM Tools
            Start-Sleep -Seconds 20
            
            #Apply latest VM Tools
            LogMessage "Applying latest VM Tools"
            $latestfile = Get-ChildItem "\\10.69.7.44\packages$\Software Distribution\ServerBuild\Server Builds 2019\VMWARE tools 12" -File | Sort-Object LastWriteTime -Descending| Select-Object -First 1
            LogMessage $latestfile.FullName
            #Disable File Open Security Warning
            $env:SEE_MASK_NOZONECHECKS = 1
            Start-Process $latestfile.FullName -ArgumentList "/v/qn REBOOT=ReallySuppress EULAS_AGREED=1"
            #Enable File Open Security Warning
            Remove-Item env:SEE_MASK_NOZONECHECKS
            Write-Host "Applied latest VM Tools" -ForegroundColor Green
            
            #Update Peer Review Script
            Write-Host "Updating Peer Review script  ...." -ForegroundColor Green
            xcopy "\\10.69.7.44\packages$\Software Distribution\ServerBuild\Peer Review Script\Peer-Review.cmd" "C:\Admin\Peer Review\" /d /f /y
                     

            #Apply MS Windows Malicious Software Removal Tool
            #Disable File Open Security Warning
            $env:SEE_MASK_NOZONECHECKS = 1
            Start-Process "\\10.69.7.44\packages$\Software Distribution\ServerBuild\Windows Malicious Software Removal Tool\Windows-KB890830-x64-V5.118.exe"
            #Enable File Open Security Warning
            Remove-Item env:SEE_MASK_NOZONECHECKS
            Write-Host "Updated MS Windows Malicious Software Removal Tool" -ForegroundColor Green
